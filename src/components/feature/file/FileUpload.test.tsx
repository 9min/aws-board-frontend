...
describe('FileUpload', () => {
  ...
  it('canEdit=true이면 파일 선택 입력과 업로드 버튼을 렌더링한다', () => {
    render(
      <FileUpload postId={1} existingAttachments={[]} onUploadComplete={mockOnUploadComplete} canEdit />,
    )
  ...
  it('canEdit=false이면 파일 선택/업로드 영역을 렌더링하지 않는다', () => {
    render(
      <FileUpload postId={1} existingAttachments={[]} onUploadComplete={mockOnUploadComplete} canEdit={false} />,
    )
